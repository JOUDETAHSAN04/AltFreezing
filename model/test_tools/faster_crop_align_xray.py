...
class FasterCropAlignXRay:
    ...
    def process_single(self, tfm, image, d, h, w):
        ...
        # Create a new image with the dimensions of (h, w, 3)
        new_image = np.zeros((h, w, 3), dtype=np.uint8)
        
        # Calculate the dimensions of the region where the image will be placed
        x, y = d
        ih, iw, _ = image.shape
        
        # Ensure that the target region has the same dimensions as the image
        target_region = new_image[y : y + ih, x : x + iw]

        # Resize the image to match the dimensions of the target region
        resized_image = cv2.resize(image, (iw, ih))
        
        # Copy the resized image into the target region
        target_region[:] = resized_image
