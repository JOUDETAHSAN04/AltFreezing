...
max_frame = 400
video_path = "examples/fakelamir.mp4"
out_dir = "prediction"
...
#     noisy = np.clip(image + gauss, 0, 255)  # Ensure pixel values are within [0, 255]
...
if __name__ == "__main__":
    ...
    for clip in tqdm(clips_for_video, desc="testing"):
        ...
        before_noise_dir = "images_before_noise"
        after_noise_dir = "images_after_noise"

        if not os.path.exists(before_noise_dir):
            os.makedirs(before_noise_dir)
        if not os.path.exists(after_noise_dir):
            os.makedirs(after_noise_dir)

        for i in range(clip_size):
            img_before_noise = images[i]
            img1 = cv2.resize(images[i], (cfg.imsize, cfg.imsize))
            img = np.concatenate((img1, images_align[i]), axis=1)
            
            # # Add Gaussian noise to the image
            # img = add_gaussian_noise(img1)
             # Apply gamma transform to the image
            gamma = 1.5  # Adjust the gamma value as needed
            img = apply_gamma_transform(img1, gamma)
            
            # Save the noisy image
            cv2.imwrite(os.path.join(after_noise_dir, f"after_noise_{frame_ids[i]}.png"), img)
            
            # Save the original image
            cv2.imwrite(os.path.join(before_noise_dir, f"before_noise_{frame_ids[i]}.png"), img_before_noise)

            # images_align[i] = img  # Update the images_align list with noisy images
